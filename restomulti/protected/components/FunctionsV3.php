...
class FunctionsV3
{
    ...
    public static function getMenu($class="menu")
    {
    	...
    	$top_menu[]=array('tag'=>"tipus",'label'=>''.Yii::t("default","Inform Us"),
                'url'=>array('/store/tipus'));
    ...
    {    	
		...
		if (isset($getdata['filter_delivery_type'])){
		    $filter_delivery_types=!empty($_GET['filter_delivery_type'])?explode(",",$_GET['filter_delivery_type']):false;
  			if (is_array($filter_delivery_types) && count($filter_delivery_types)>=1){
				$x=1;
				foreach ($filter_delivery_types as $val) {				
					if (!empty($val)){
						switch ($val) {
							case 1:
								$realStr = 'Volledig halal';
								break;			
							case 2:
								$realStr = 'Deels halal';
								break;
							case 3:
								...
								break;		
							case 4:
								$realStr = 'Vis beschikbaar';
								break;		
							case 5:
								$realStr = 'Vegetarisch beschikbaar';
								break;		
							default:
								break;
						}
						if ( $x==1){
							$filter_delivery_type.=" LIKE '%".$realStr."%'";
						} else $filter_delivery_type.=" OR halal LIKE '%".$realStr."%'";
						$x++;
					}
				}				
				if (!empty($filter_delivery_type)){
				   $and.=" AND (halal $filter_delivery_type) ";
				}
			}
		}		
		...
		$service = "";
	    $service_id = isset($getdata['service'])?$getdata['service']:'';
	    if (!empty($service_id)) {
	    	switch ($service_id) {
	    		case 1:
	    		$service .= "AND (service = 1 OR service = 4 OR service = 5) ";
	    			break;
	    		case 2:
	    		$service .= "AND (service = 2 OR service = 4 OR service = 5) ";
	    			break;
	    		case 3:
	    		$service .= "AND (service = 3 OR service = 5) ";
	    			break;
	    		case 4:
	    		$service .= "AND (service = 1 OR service = 2 OR service = 4) ";
	    			break;
	    		case 5:
	    		$service .= "AND (service = 1 OR service = 2 OR service = 3 OR service = 4 OR service = 5) ";
	    			break;
	    		default:
	    			break;
	    	}
	   	    //$service .= "AND service LIKE '%$service_id%' ";
	    }
    ...
    {        
		...
		switch ($stype) {
			...
			case "kr_postcode":   
			   ...
			   $service_id = isset($getdata['service'])?$getdata['service']:'';
			   if (!empty($service_id)) {
			   	$query .= "AND service LIKE '%$service_id%' ";
			   }
			   break;
    ...
    public function getHalal($merchant_id = '') {
        $DbExt = new DbExt;
        $stmt = "SELECT * FROM
		{{merchant}}
		WHERE
    ...
    public function getHalalClass() {
        ...
        $data = [];
        $data[] = ['img' => $path . 'halal.png'];
        $data[] = ['img' => $path . 'certificaat.png'];
        $data[] = ['img' => $path . 'alcohol.png'];
        $data[] = ['img' => $path . 'shisha.png'];
        return $data;
    }
    ...
    public static function getAdminPaymentList()
    {
    	...
    	if (getOptionA('admin_mollie_enabled')==""){
    ...
    public static function getMerchantPaymentList($merchant_id='')
    {
		...
		if ( Yii::app()->functions->isMerchantCommission($merchant_id)){
			...
			if ( getOption($merchant_id,'enabled_mollie')=="yes"){
				$payment_available[]='mol';
			}
			...
			//print_r($admin_available_payment);
			//print_r($payment_list);
			//print_r($payment_available);
