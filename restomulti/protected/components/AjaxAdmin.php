...
$params['cart_tip_value']=isset($this->data['cart_tip_value'])?$this->data['cart_tip_value']:'';                        			            
		...
		{		    
		    ...
		    $params=array(
		      'restaurant_name'=>addslashes($this->data['restaurant_name']),
...
$last_login=$val['last_login']=="0000-00-00 00:00:00"?"":date('M d,Y G:i:s',strtotime($val['last_login']));  */
		...
		{			
			$merchant_id=Yii::app()->functions->getMerchantID();
			if (count($this->data['photo']) > 5) {
				$this->msg=Yii::t("default","ERROR: cannot update over 5 pictures.");
			} else {
...
if (!Yii::app()->functions->getMerchantOffers($merchant_id,$this->data['valid_from']." 00:00:00",$this->data['valid_to']." 00:00:00")){
        ...
        public function saveAdminMollieSettings() {
	    	$merchant_id=Yii::app()->functions->getMerchantID();
	    	
	    	Yii::app()->functions->updateOption("enabled_mollie",
	    	isset($this->data['admin_mollie_enabled'])?$this->data['admin_mollie_enabled']:'',$merchant_id);

            Yii::app()->functions->updateOptionAdmin("admin_mollie_api_key",
                isset($this->data['admin_mollie_api_key']) ? $this->data['admin_mollie_api_key'] : '');

            Yii::app()->functions->updateOption("mollie_api_key",
                isset($this->data['admin_mollie_api_key']) ? $this->data['admin_mollie_api_key'] : '',$merchant_id);

            Yii::app()->functions->updateOptionAdmin("admin_mollie_enabled", 
            	isset($this->data['admin_mollie_enabled']) ? $this->data['admin_mollie_enabled'] : '' );
