        ...
        success: function(data){ 
        	...
        	if (data.code==1){
        		...
        		if ( action == "placeOrder" ){
                    ...
                    console.log(data.details.payment_type);
                    if (data.details.payment_type == "mol" || data.details.payment_type == "mollie") {
                        window.location.replace(sites_url + "/store/mollieInit/id/" + data.details.order_id);
...
jQuery(document).ready(function() {	
    ...
    if ( $(".icheck").exists() ) {
        var iCheckName = $('.icheck').attr('name');

        if (iCheckName == 'tip_opt') {
         $('.icheck').iCheck({
...
/** START TIP US VERSION */
jQuery(document).ready(function() {
    $('.tip_option').on('ifChecked', function(event){               
        var seleted_tip=$(this).val();
        if (seleted_tip == 'tip_one') {
            $('#tip-st').addClass('tip-hidden');
            $('#tip-zip').addClass('tip-hidden');
            $('#tip-city').addClass('tip-hidden');
            $('#tip-phone').addClass('tip-hidden');
        } else {
            $('#tip-st').removeClass('tip-hidden');
            $('#tip-zip').removeClass('tip-hidden');
            $('#tip-city').removeClass('tip-hidden');
            $('#tip-phone').removeClass('tip-hidden');
        }
        dump(seleted_tip);
    });
});
